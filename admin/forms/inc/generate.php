...
function nb_createinput($type,$options){
	...
	switch($type) {
		...
		case 'smalltext':
			$html = '<label class="textinput" for="' . $options['name'] . '">' . $options['label'] . $mandatory .': </label>
			<input id="' . $options['id']. '" type="text" class="colorinput" size="' . $options['size'] . '" name="' . $options['name']. '" value="' . $options['sel'] . '" />';
			break;
		case 'multitext':
			$html = '<label class="textinput" for="' . $options['name'] . '">' . $options['label'] . $mandatory .': </label>
		...
		case 'notusedyet':
			...
			<input type="button" class="' . $options['id'] . '-pickcolor button hide-if-no-js" value="' . __( 'Select a Color', 'attention-bar' ) . '" />
			<div id="' . $options['id'] . '-colorpicker" style="z-index: 100; background:#eee; border:1px solid #ccc; position:absolute; display:none;"></div>
			<!--<span>' . __('Default color:','attention-bar') . ' <span id="' . $options['id'] . '-default-color"><a href="#">#ffffff</a></span></span>-->';
