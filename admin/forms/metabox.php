...
function nb_add_custom_metabox() {
    add_meta_box( 
        'pd_nb_metabox',
        __( 'Attention bar', 'attention-bar' ),
...
function nb_generate_innermeta() {
  ...
  echo '<select name="pd_nb_metastatus">
	<option value="true" ' . selected($value,'true',false) . '>' . __('Enabled','attention-bar') . '</option>
	<option value="false" ' . selected($value,'false',false) . '>' . __('Disabled','attention-bar') . '</option>
	</select>
	';
}
