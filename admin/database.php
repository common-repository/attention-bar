...
function nb_insert_entry() {
	...
	$author = nb_get_currentuserid();
	if(empty($begin) || empty($end)) {
		$wpdb->insert( $table_name, array( 'author' => $author, 'status' => 0, 'timecreated' => current_time('mysql',0), 'name' => $name, 'content' => $content, 'status' => $status, 'bgcolor' => $bgcolor, 'fontcolor' => $fontcolor, 'nottype' => $nottype ) );
	} else {
        $wpdb->insert( $table_name, array( 'author' => $author, 'status' => 0, 'timecreated' => current_time('mysql',0), 'name' => $name, 'content' => $content, 'status' => $status, 'epochtimebegin' => (int)strtotime($begin), 'epochtimeend' => (int)strtotime($end), 'bgcolor' => $bgcolor, 'fontcolor' => $fontcolor, 'nottype' => $nottype ) );
	}
}
