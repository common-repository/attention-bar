...
function nb_get_content() {
	...
	if (get_option('pd_nb_debug','false') == 'true') {
        ...
        if (count($myrows) != 0) {
			...
			if (empty($tmpnottext)) {
				$annmessages .= '"' . get_option('pd_nb_bartext', 'IMPORTANT!') . '",';
			} else {
			...
			if (empty($myrows->nottype)) {
				...
				$tmpnoticon = $wpdb->get_results( "SELECT icon,name FROM " . $table_notifications . " WHERE id = " . $myrows->nottype );
	...
	if (get_option('pd_nb_showversion','true') == 'true' || get_option('pd_nb_debug','false') == 'true') {
		return chr(13) . '<!-- Attention bar: ' . NB_VERSION . ' START -->' . chr(13) . $content . '<!-- Attention bar: ' . NB_VERSION . ' END -->';
	} else {
		return $content;
	}
}
